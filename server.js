...
app.get("/admin/payments", adminAuth, async (req, res) => {
  try {
    ...
    res.json({ success: true, data: payments, page, totalPages: Math.max(1, Math.ceil(total / limit)), total });
  } catch (error) { res.status(500).json({ success: false, error: error.message }); }
});
...
app.get("/admin/users/:email/details", adminAuth, async (req, res) => {
  try {
    ...
    const [totalTranscriptions, totalClipJobs, lastReel, lastClipJob, user] = await Promise.all([
      Reel.countDocuments({ userEmail: email }),
      ClipJob.countDocuments({ userEmail: email }),
      Reel.findOne({ userEmail: email }).sort({ createdAt: -1 }).select("createdAt"),
      ClipJob.findOne({ userEmail: email }).sort({ createdAt: -1 }).select("createdAt"),
      User.findOne({ email }).select("lastTranscriptDate lastClipDate"),
    ]);

    const candidates = [
      lastReel?.createdAt,
      lastClipJob?.createdAt,
      user?.lastTranscriptDate,
      user?.lastClipDate,
    ].filter(Boolean).map(d => new Date(d).getTime());

    const lastActive = candidates.length ? new Date(Math.max(...candidates)) : null;

    res.json({
      success: true,
      details: { totalTranscriptions, totalClipJobs, lastActive },
    });
  } catch (error) { res.status(500).json({ success: false, error: error.message }); }
});
...
app.post("/admin/credit", adminAuth, async (req, res) => {
  try {
    ...
    const validActions = ["add", "deduct", "set", "reset"];
    if (!validActions.includes(action)) return res.status(400).json({ success: false, error: "Invalid action" });

    const amt = action === "reset" ? 0 : parseInt(amount);
    if (action !== "reset" && (!Number.isInteger(amt) || amt < 0)) {
      return res.status(400).json({ success: false, error: "Enter a whole-number credit amount." });
    }

    let update, message;
    if (action === "add") {
      update = { $inc: { credits: amt } };
      message = `Added ${amt} credits`;
    } else if (action === "deduct") {
      update = { $inc: { credits: -amt } };
      message = `Deducted ${amt} credits`;
    } else if (action === "set") {
      update = { credits: amt };
      message = `Credits set to ${amt}`;
    } else {
      update = { credits: 0 };
    ...
    // Deduct/set could push credits below 0 — clamp it back to 0.
    if (user.credits < 0) user = await User.findOneAndUpdate({ email }, { credits: 0 }, { new: true });

    logAdminAction("credit-" + action, email, `${message} (new total: ${user.credits})`, req);
    res.json({ success: true, message: `${message} successfully.`, user });
  } catch (error) { res.status(500).json({ success: false, error: error.message }); }
});
...
app.post("/admin/user-control", adminAuth, async (req, res) => {
  try {
    ...
    const validActions = ["suspend", "unsuspend", "delete"];
    if (!validActions.includes(action)) return res.status(400).json({ success: false, error: "Invalid action" });

    if (action === "delete") {
      const user = await User.findOneAndDelete({ email });
      if (!user) return res.status(404).json({ success: false, error: "User not found" });
      logAdminAction("delete-user", email, "User account deleted", req);
    ...
    const user = await User.findOneAndUpdate(
      { email },
      { isSuspended, suspendedAt: isSuspended ? new Date() : null },
      { new: true }
    );
    if (!user) return res.status(404).json({ success: false, error: "User not found" });

    logAdminAction(action + "-user", email, `User ${isSuspended ? "suspended" : "unsuspended"}`, req);
    res.json({ success: true, message: `User ${isSuspended ? "suspended" : "unsuspended"} successfully.`, user });
  } catch (error) { res.status(500).json({ success: false, error: error.message }); }
});
...
app.post("/admin/set-plan", adminAuth, async (req, res) => {
  try {
    ...
    // Preserve which paid plan the user last had, for churn / win-back
    // detection on the admin panel — don't wipe this on downgrade.
    let lastPaidPlan = existingUser.lastPaidPlan || null;
    if (plan === "free" && existingUser.plan && existingUser.plan !== "free") {
      lastPaidPlan = existingUser.plan;
    } else if (plan !== "free") {
      lastPaidPlan = null; // back on a paid plan, no longer churned
    }
