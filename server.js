...
passport.use(new GoogleStrategy({
  ...
  try {
    ...
    if (!user) {
      ...
      user = await User.create({ name: profile.displayName, email, credits: 5, referredBy: referralCode });
      if (referralCode) await createReferralRecord(referralCode, email);
...
async function createReferralRecord(referralCode, newUserEmail) {
...
async function creditReferralAfterFirstTranscript(user) {
  ...
  const referrerEmail = (await User.findOne({ referralCode: String(user.referredBy).toUpperCase() }).select("email").lean())?.email?.toLowerCase();
...
app.post("/verify-otp", async (req, res) => {
  try {
    ...
    if (!user) {
      const referralCode = req.session.referralCode || null;
      user = await User.create({ name: email.split("@")[0], email, credits: 5, referredBy: referralCode });
      if (referralCode) await createReferralRecord(referralCode, email);
      delete req.session.referralCode;
    }
...
app.post("/transcribe", upload.single("video"), async (req, res) => {
  try {
    ...
    if (!isGuest) {
      ...
      await creditReferralAfterFirstTranscript(user);
